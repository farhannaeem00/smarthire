    ...
    <div className="min-h-screen bg-[#0B0E0D] text-white">
      ...
      <div className="border-b border-white/5 px-6 py-4 flex items-center justify-between sticky top-0 bg-[#0B0E0D]/95 backdrop-blur z-40">
        ...
        <div className="flex items-center gap-4">
          ...
          <span className="text-xs bg-emerald-500/10 text-emerald-400 px-2 py-1 rounded-full">Candidate</span>
          <button
            ...
            {applications.length === 0 ? (
              ...
              <div className="space-y-4">
                {applications.map((app) => (
                  <div key={app.id} className="bg-[#111513] border border-white/10 rounded-2xl p-6">
                    <div className="flex items-start justify-between gap-4">
                      ...
                      <div className="text-center shrink-0">
                        <p className={`text-3xl font-bold ${getScoreColor(app.ai_score)}`}>{app.ai_score}</p>
                    ...
                    {app.ai_questions && (
                      <div className="mt-3">
                        ...
                        <div id={`questions-${app.id}`} className="hidden mt-2 space-y-2">
                          {JSON.parse(app.ai_questions.replace(/'/g, '"')).map((q: string, i: number) => (
        ...
        {activeTab === "profile" && (
          <div className="bg-[#111513] border border-white/10 rounded-2xl p-6">
            ...
            <div className="flex items-center gap-4 mb-6">
              <div className="w-16 h-16 bg-emerald-500/10 rounded-full flex items-center justify-center text-2xl font-bold text-emerald-400">
                {user?.name?.[0]?.toUpperCase()}
              </div>
              <div>
                <h3 className="font-bold text-xl">{user?.name}</h3>
                <p className="text-gray-400">{user?.email}</p>
                <span className="text-xs bg-emerald-500/10 text-emerald-400 px-2 py-0.5 rounded-full">Candidate</span>
              </div>
            </div>
            <Link href="/profile"
              className="bg-emerald-500 hover:bg-emerald-400 text-black font-semibold px-6 py-2.5 rounded-lg transition">
              Edit Profile
            </Link>
          </div>
        )}
