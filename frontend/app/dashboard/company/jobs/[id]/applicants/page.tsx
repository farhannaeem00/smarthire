  ...
  const fetchData = async () => {
    try {
      const [jobRes, appsRes] = await Promise.all([
        getJob(id as string),
        getJobApplications(id as string),
      ]);
            ...
            {applications.length === 0 ? (
              ...
              applications.map((app, index) => (
                <div
                  ...
                  className={`bg-gray-900 border rounded-2xl p-5 cursor-pointer transition ${
                ...
                >
                  <div className="flex items-center justify-between">
                    <div className="flex items-center gap-3">
                      <div className="w-10 h-10 bg-blue-500/10 rounded-full flex items-center justify-center font-bold text-blue-400">
                        #{index + 1}
                      </div>
                      <div>
                    ...
                    <div className="text-right">
                      <p className={`text-2xl font-bold ${getScoreColor(app.ai_score)}`}>
                        {app.ai_score}
                      </p>
                  ...
                  <div className="flex items-center justify-between mt-3">
                    ...
                    <div className="flex gap-1">
                      {["shortlisted", "rejected", "hired"].map((s) => (
                        <button
                          key={s}
                          onClick={(e) => { e.stopPropagation(); handleStatusUpdate(app.id, s); }}
            ...
            {selectedApp ? (
              <div className="bg-gray-900 border border-gray-800 rounded-2xl p-6 sticky top-24 space-y-5">
                <div className="flex items-center justify-between">
                  ...
                  <span className={`text-xs px-2.5 py-1 rounded-full ${getStatusColor(selectedApp.status)}`}>
                    {selectedApp.status}
                  </span>
                </div>
                  ...
                  <div className="flex items-center gap-4">
                    <div className="relative w-16 h-16">
                      <svg className="w-16 h-16 -rotate-90" viewBox="0 0 36 36">
                        ...
                        <path d="M18 2.0845 a 15.9155 15.9155 0 0 1 0 31.831 a 15.9155 15.9155 0 0 1 0 -31.831"
                          fill="none"
                          stroke={selectedApp.ai_score >= 70 ? "#22c55e" : selectedApp.ai_score >= 40 ? "#f59e0b" : "#ef4444"}
                          strokeWidth="3"
                          strokeDasharray={`${selectedApp.ai_score}, 100`} />
                      </svg>
                    ...
                    <div>
                      ...
                      <p className={`text-sm ${getScoreColor(selectedApp.ai_score)}`}>
                        {selectedApp.ai_score >= 70 ? "Strong Match 🎉" :
                         selectedApp.ai_score >= 40 ? "Good Potential 👍" : "Weak Match ⚠️"}
                      </p>
                    </div>
                  </div>
                </div>
                ...
                {/* Skills Match */}
                <div>
                  <p className="text-sm font-semibold text-blue-400 mb-2">🎯 Skills Analysis</p>
                ...
                {selectedApp.cover_letter && (
                  <div>
                    <p className="text-sm font-semibold text-blue-400 mb-2">📝 Cover Letter</p>
                    <p className="text-gray-300 text-sm leading-relaxed bg-gray-800 rounded-xl p-3">
                      {selectedApp.cover_letter}
                    </p>
                  </div>
                )}
                ...
                {selectedApp.ai_questions && (
                  <div>
                    <p className="text-sm font-semibold text-blue-400 mb-2">❓ AI Interview Questions</p>
                    <div className="space-y-2">
                      {(() => {
                        try {
                          const questions = JSON.parse(
                            selectedApp.ai_questions.replace(/'/g, '"')
                          );
                          ...
                          return <p className="text-gray-400 text-xs">Questions unavailable</p>;
                        }
                      })()}
                    </div>
                  </div>
                ...
                <div>
                  ...
                  <div className="grid grid-cols-2 gap-2">
                    {[
                      ...
                      { value: "shortlisted", label: "Shortlist", color: "bg-green-500/10 text-green-400" },
                      ...
                      <button
                        key={s.value}
                        onClick={() => handleStatusUpdate(selectedApp.id, s.value)}
