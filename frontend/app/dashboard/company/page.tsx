...
import { getCompanyByUser, getCompanyJobs, getCompanyStats, deleteJob, updateJob, updateCompany } from "@/lib/api";
...
export default function CompanyDashboard() {
  ...
  const fetchData = async (userId: string) => {
    try {
      ...
      if (!companyData.company) { router.push("/signup?role=company"); return; }
    ...
    <div className="min-h-screen bg-[#0B0E0D] text-white">
      ...
      <div className="border-b border-white/5 px-6 py-4 flex items-center justify-between sticky top-0 bg-[#0B0E0D]/95 backdrop-blur z-40">
        ...
        <div className="flex items-center gap-4">
          ...
          <span className="text-xs bg-emerald-500/10 text-emerald-400 px-2 py-1 rounded-full">Company</span>
          <button
      ...
      <div className="max-w-6xl mx-auto px-6 py-8">
        ...
        <div className="flex items-center justify-between mb-8 flex-wrap gap-4">
          ...
          <Link href="/dashboard/company/post-job"
            className="bg-emerald-500 hover:bg-emerald-400 text-black font-semibold px-5 py-2.5 rounded-lg transition">
            + Post Job
          </Link>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-8">
          {[
            ...
            <div key={stat.label} className="bg-[#111513] border border-white/10 rounded-2xl p-6">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
            <div className="bg-[#111513] border border-white/10 rounded-2xl p-6">
              ...
              {jobs.length === 0 ? (
                <div className="text-center py-8">
                  ...
                  <Link href="/dashboard/company/post-job" className="text-emerald-400 text-sm hover:underline mt-2 block">
                    Post your first job
                  </Link>
                </div>
              ) : (
                  ...
                  {jobs.slice(0, 4).map((job) => (
                    <div key={job.id} className="flex items-center justify-between p-3 bg-white/5 rounded-xl">
                      ...
                      <div className="flex items-center gap-2">
                        ...
                        <Link href={`/dashboard/company/jobs/${job.id}/applicants`} className="text-xs text-emerald-400 hover:underline">
                          View →
                        </Link>
                      </div>
                    </div>
            ...
            <div className="bg-[#111513] border border-white/10 rounded-2xl p-6">
              ...
              <div className="space-y-3 text-sm">
                <div className="flex justify-between"><span className="text-gray-400">Company</span><span>{company?.name}</span></div>
                <div className="flex justify-between"><span className="text-gray-400">Industry</span><span>{company?.industry || "—"}</span></div>
                <div className="flex justify-between"><span className="text-gray-400">Location</span><span>{company?.location || "—"}</span></div>
                <div className="flex justify-between"><span className="text-gray-400">Website</span><span className="text-emerald-400">{company?.website || "—"}</span></div>
                <div className="flex justify-between"><span className="text-gray-400">Size</span><span>{company?.size || "—"}</span></div>
              </div>
              <button onClick={() => setActiveTab("company")} className="mt-4 text-sm text-emerald-400 hover:underline">
                Edit Company Info →
              </button>
            </div>
          </div>
          ...
          <div>
            {jobs.length === 0 ? (
              <div className="text-center py-20">
                ...
                <Link href="/dashboard/company/post-job"
                  className="bg-emerald-500 hover:bg-emerald-400 text-black font-semibold px-6 py-3 rounded-xl transition">
                  Post First Job
                </Link>
              </div>
            ) : (
              <div className="space-y-4">
                {jobs.map((job) => (
                  <div key={job.id} className="bg-[#111513] border border-white/10 rounded-2xl p-6">
                    <div className="flex items-start justify-between flex-wrap gap-4">
                      <div className="flex-1">
                        ...
                        <div className="flex gap-2 mt-3 flex-wrap">
                          {job.skills?.slice(0, 4).map((skill: string) => (
                            <span key={skill} className="text-xs bg-emerald-500/10 text-emerald-400 px-2 py-1 rounded-full">
                              {skill}
                            </span>
                          ))}
                        </div>
                      ...
                      <div className="flex flex-col gap-2 shrink-0">
                        <Link href={`/dashboard/company/jobs/${job.id}/applicants`}
                          className="text-xs bg-emerald-500 hover:bg-emerald-400 text-black px-3 py-1.5 rounded-lg transition text-center font-semibold">
                          View Applicants
                        </Link>
                        <button onClick={() => handleToggleJob(job.id, job.is_active)}
                          className="text-xs bg-white/5 hover:bg-white/10 text-white px-3 py-1.5 rounded-lg transition">
...
function CompanyEditForm({ company, onUpdate }: { company: any; onUpdate: (c: any) => void }) {
  ...
  const handleSave = async () => {
    setSaving(true);
    try {
      const { data } = await updateCompany(company.id, { name, description, website, industry, size, location });
  ...
  return (
    <div className="bg-[#111513] border border-white/10 rounded-2xl p-6 max-w-2xl">
      ...
      <div className="space-y-4">
        ...
        <button onClick={handleSave} disabled={saving}
          className="w-full bg-emerald-500 hover:bg-emerald-400 text-black font-semibold py-2.5 rounded-lg transition disabled:opacity-50">
