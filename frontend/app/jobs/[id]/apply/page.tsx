...
export default function ApplyPage() {
  ...
  useEffect(() => {
    const u = getUser();
    if (!u || u.role !== "candidate") { router.push("/login"); return; }
    setUser(u);
    fetchJob();
  }, [id]);
  ...
  const handleSubmit = async () => {
    ...
    if (!resume.name.endsWith(".pdf")) { toast.error("Only PDF files are allowed"); return; }

    setSubmitting(true);
    try {
      const formData = new FormData();
      formData.append("job_id", id as string);
      formData.append("candidate_id", user.id || user._id);
    ...
    <div className="min-h-screen bg-[#0B0E0D] text-white">
      ...
      <div className="max-w-3xl mx-auto px-6 py-10">
        {aiResult ? (
          <div className="space-y-6">
              ...
              <div className="flex items-center gap-4 mb-4">
                <div className="relative w-24 h-24">
                  <svg className="w-24 h-24 -rotate-90" viewBox="0 0 36 36">
                    ...
                    <path d="M18 2.0845 a 15.9155 15.9155 0 0 1 0 31.831 a 15.9155 15.9155 0 0 1 0 -31.831" fill="none"
                      stroke={aiResult.ai_score >= 70 ? "#10b981" : aiResult.ai_score >= 40 ? "#f59e0b" : "#ef4444"}
                      strokeWidth="3" strokeDasharray={`${aiResult.ai_score}, 100`} />
                  </svg>
                ...
                <div>
                  <p className={`text-xl font-bold ${
                    aiResult.ai_score >= 70 ? "text-emerald-400" : aiResult.ai_score >= 40 ? "text-yellow-400" : "text-red-400"
                  }`}>
                    {aiResult.ai_score >= 70 ? "Strong Match! 🎉" : aiResult.ai_score >= 40 ? "Good Potential 👍" : "Needs Improvement 📚"}
                  </p>
                ...
                <div className="space-y-3">
                  {aiResult.ai_questions.map((q: string, i: number) => (
                    <div key={i} className="flex items-start gap-3 p-3 bg-white/5 rounded-xl">
                      <span className="text-emerald-400 font-bold text-sm shrink-0">Q{i + 1}.</span>
            ...
            <div className="flex gap-3">
              <Link href="/dashboard/candidate"
                className="flex-1 text-center bg-emerald-500 hover:bg-emerald-400 text-black font-semibold py-3 rounded-xl transition">
                View My Applications
              </Link>
              <Link href="/jobs"
                className="flex-1 text-center border border-white/15 hover:border-white/30 text-gray-300 font-semibold py-3 rounded-xl transition">
                Browse More Jobs
              </Link>
            </div>
          </div>
          ...
          <>
            <div className="bg-[#111513] border border-white/10 rounded-2xl p-5 mb-6">
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  <p className="text-sm text-gray-400">{job?.companies?.name || job?.company?.name} • {job?.location}</p>
                </div>
              </div>
            </div>
            ...
            <div className="space-y-6">
              <div className="bg-[#111513] border border-white/10 rounded-2xl p-6">
                ...
                <p className="text-gray-400 text-sm mb-4">Our AI will analyze your resume and score it against the job requirements.</p>
                <div className="border-2 border-dashed border-white/10 rounded-xl p-8 text-center">
                  <input type="file" accept=".pdf" onChange={(e) => setResume(e.target.files?.[0] || null)} className="hidden" id="resume-upload" />
                  <label htmlFor="resume-upload" className="cursor-pointer">
                    <p className="text-4xl mb-3">📄</p>
                    <p className="text-gray-400 text-sm">
                      {resume ? <span className="text-emerald-400 font-semibold">✓ {resume.name}</span> : "Click to upload your resume PDF"}
                    </p>
              ...
              <div className="bg-[#111513] border border-white/10 rounded-2xl p-6">
                ...
                <textarea
                  ...
                  rows={5}
                  className="w-full bg-white/5 border border-white/10 text-white rounded-lg px-4 py-3 focus:outline-none focus:border-emerald-500 resize-none text-sm"
                />
              </div>

              <div className="bg-emerald-500/10 border border-emerald-500/20 rounded-xl p-4">
                <p className="text-emerald-300 text-sm font-semibold mb-1">🧠 AI Powered Application</p>
                <p className="text-emerald-400/80 text-xs">
                  After submitting, our AI will instantly analyze your resume, give you a match score, identify skills gaps, and generate personalized interview questions to help you prepare.
                </p>
              </div>
